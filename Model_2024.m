...
mean_rate = moving_average(OUTPUT);
% 
...
% [Time,Reorientations,M] = gillespie_search(fitparameters,time(end),mod_initial);
[Time,Reorientations,M] = gillespie_search(fitparameters,time(end),d2);
...
Reorientation_rate = alpha*exp(-gamma*time);
...
function [Time,Reorientations,M] = gillespie_search(parameters,time_limit,number)
...
for cycle =1:number
    ...
    while T(count,1) < time_limit % Do the simulation for 50 min window
        ...
        a = [(alpha*Xstart(2)),gamma*X(count,2)];
        a0 = sum(a);
        r = rand(1,2);
