...
% gamma are optimized so that the residual of the cumulative reorientation
% data and cumulative model data is minimized.
% 
...
% Fit parameters to mean of data
meanorient = mean(data,2); % mean cumulative number of reorientations from experiment.
fitparameters = nlinfit(time',meanorient,@ratemodel,parameters);
...
[Time,Reorientations] = gillespie_search(fitparameters,d2,time(end));
...
% Grab inter-orientation time intervals for exp
...
% MODEL %
%-------%
function Orientations = ratemodel(parameters,time)

alpha = parameters(1);
gamma = parameters(2);

Orientations = (alpha/gamma)*(1 - exp(-gamma*time));
...
function [Time,Reorientations] = gillespie_search(parameters,number,time_limit)
...
for cycle =1:number
    ...
    while T(count,1) < time_limit % Do the simulation for 50 min window
        ...
        a = [(alpha*X(count,2)),gamma*X(count,2)];
        a0 = sum(a);
        r = rand(1,2);
